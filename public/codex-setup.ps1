...
Run:  irm https://cdn.deepseek.com/api-docs/codex-deepseek-setup-en.ps1 | iex
...
$SCRIPT_URL  = 'https://cdn.deepseek.com/api-docs/codex-deepseek-setup-en.ps1'
...
function Assert-ModelsJson {
    ...
    if ($slugs.Count -ne $want.Count -or @($want | Where-Object { $slugs -notcontains $_ }).Count -gt 0) {
        throw "models.json must contain exactly two models, $FLASH_SLUG and $PRO_SLUG"
    }
}
...
function Invoke-DeepSeekRestore {
    ...
    Write-Head 'Restore the default Codex configuration (remove deepseek settings)'
    ...
    Write-Host ''
    Write-Warn2 'Fully quit the ChatGPT desktop app and reopen it, or the restore will not take effect'
...
function Invoke-DeepSeekSwitchModel {
    ...
    Write-Head "Updating the configuration -> $($script:ModelSlug)"
    Write-Dim  'Backup from this script detected; models.json will be refreshed and the model field in config.toml updated.'
    ...
    if ($hadLegacy) {
        Write-Dim "Model entries named $LEGACY_SLUG_PREFIX* from an older version were found; they will be removed, leaving $FLASH_SLUG and $PRO_SLUG."
    }
    ...
    try {
        ...
        Die "The generated models.json failed validation; aborted (the original files were not modified).`n$($_.Exception.Message)"
    }
    ...
    if ($hadLegacy) {
        Write-Ok "models.json refreshed (the old $LEGACY_SLUG_PREFIX* entries were removed; it now contains $FLASH_SLUG and $PRO_SLUG)"
    } else {
        Write-Ok "models.json refreshed (contains $FLASH_SLUG and $PRO_SLUG)"
    }
    Write-Dim '     (this file is rewritten from the catalog embedded in the script; manual edits to it are overwritten)'
    Write-Ok "config.toml updated: model = `"$($script:ModelSlug)`", web_search = `"disabled`""
    Write-Host ''
    Write-Warn2 'Fully quit the ChatGPT desktop app and reopen it, or the change will not take effect'
    Write-Host '     (right-click the taskbar tray icon and choose Quit; closing the window leaves it running)'
    Write-Host ''
    Write-Host 'How to verify it took effect:'
    Write-Host '  - ChatGPT desktop app: after launch, the bottom-right corner showing "Custom" or the selected model name both mean success'
    Write-Host '    (which one depends on your ChatGPT version; when it shows the model name you can switch models right in the app)'
    Write-Host "  - Codex CLI: the startup banner shows model: $($script:ModelSlug)"
    Write-Host ''
    Write-Dim 'Run this script again to rewrite the configuration (pick 1/2) or restore the default config (pick 9).'
}
...
function Invoke-DeepSeekInstall {
    ...
    if ($ApiKey) {
        ...
        Write-Host ''
        Write-Ok 'Using the API key from the DEEPSEEK_API_KEY environment variable; skipping the prompt.'
    }

    Write-Host ''
    if (-not $ApiKey) {
        Write-Dim "Don't have an API key yet? Create one at https://platform.deepseek.com/api_keys"
    }
    $keyAttempt = 0
    while (-not $ApiKey) {
        try {
            $ApiKey = (Read-Host 'Enter your DeepSeek API key (starts with sk-)').Trim()
        } catch {
...
"@
    ...
    Write-Host ''
    Write-Host '  Pick 1/2 to rewrite the configuration, or 9 to restore the pre-install configuration.'
    Write-Host ''
}
...
"@
    ...
    Write-Host '  9. Restore the default Codex configuration (remove deepseek settings)'
    Write-Host ''
    ...
    while ($true) {
        ...
        Write-Warn2 'Invalid input; please enter 1, 2 or 9.'
    }
    ...
    if (Test-Path -LiteralPath $BackupDir) {
        ...
        if (-not (Test-Path -LiteralPath $ModelsPath)) {
            ...
            if ($modelsRaw -notmatch '"deepseek') {
                $problems += "  - $ModelsPath does not contain any deepseek model entry"
            }
        }
...
Suggested fixes (pick one):
  a) Run this script again, pick 9 to restore the default configuration first,
